...
constexpr std::array<uint8_t, 8> kMagic = {'I', 'W', 'E', 'B', 'P', 'I', 'D', 'X'};
...
struct WebpApi {
    ...
    using DecodeRgbaIntoFn = uint8_t* (*)(
        const uint8_t*, size_t, uint8_t*, size_t, int
    );

    void* library = nullptr;
    GetInfoFn getInfo = nullptr;
    DecodeRgbaIntoFn decodeRgbaInto = nullptr;

    WebpApi() {
        library = dlopen("libglide-webp.so", RTLD_NOW | RTLD_LOCAL);
        if (library == nullptr) {
            throw std::runtime_error(std::string("Unable to load libwebp: ") + dlerror());
        }
        getInfo = reinterpret_cast<GetInfoFn>(dlsym(library, "WebPGetInfo"));
        decodeRgbaInto = reinterpret_cast<DecodeRgbaIntoFn>(dlsym(library, "WebPDecodeRGBAInto"));
        if (getInfo == nullptr || decodeRgbaInto == nullptr) {
            throw std::runtime_error("The packaged libwebp lacks required decode symbols");
        }
    }
...
void logError(const char* stage, const std::string& message) {
    __android_log_print(ANDROID_LOG_ERROR, "IndexedWebp", "%s: %s", stage, message.c_str());
}
...
std::vector<Level> makeLevels(uint32_t width, uint32_t height) {
    if (width == 0 || height == 0) throw std::runtime_error("WebP dimensions are empty");
    std::vector<Level> levels;
    uint32_t sample = 1;
    size_t firstEntry = 0;
    while (true) {
        ...
        if (count > kMaxEntries || firstEntry > kMaxEntries - count) {
            throw std::runtime_error("WebP tile directory is too large");
        }
        ...
        if (sample > std::numeric_limits<uint32_t>::max() / 2) {
            throw std::runtime_error("WebP pyramid has too many levels");
        }
        sample *= 2;
    }
    return levels;
...
void writeTile(FILE* output, Entry& entry, const std::vector<uint8_t>& raw) {
    if (raw.size() != entry.rawBytes) throw std::runtime_error("WebP tile size mismatch");
...
void writeBand(
    ...
    for (uint32_t tileX = 0; tileX < level.columns; ++tileX) {
        Entry& entry = entries[level.firstEntry + static_cast<size_t>(tileY) * level.columns + tileX];
        if (entry.height != bandHeight) throw std::runtime_error("WebP band height mismatch");
...
std::vector<uint8_t> loadTileRaw(int fd, const Entry& entry) {
    ...
    if (!preadAll(fd, compressed.data(), compressed.size(), entry.offset)) {
        throw std::runtime_error("Unable to read WebP index tile");
    }
    ...
    if (result != Z_OK || rawSize != raw.size()) {
        throw std::runtime_error("Unable to inflate WebP index tile");
    }
    const uint32_t actualCrc = crc32(0, raw.data(), static_cast<uInt>(raw.size()));
    if (actualCrc != entry.crc) throw std::runtime_error("WebP index tile checksum mismatch");
    return raw;
}
...
void generateLowerLevels(FILE* output, const std::vector<Level>& levels, std::vector<Entry>& entries) {
    ...
    for (size_t levelIndex = 1; levelIndex < levels.size(); ++levelIndex) {
        if (fflush(output) != 0) throw std::runtime_error("Unable to flush WebP index level");
            ...
            for (uint32_t tileX = 0; tileX < destination.columns; ++tileX) {
                ...
                for (uint32_t y = 0; y < outputEntry.height; ++y) {
                    for (uint32_t x = 0; x < outputEntry.width; ++x) {
                        ...
                        if (count == 0) throw std::runtime_error("WebP pyramid source pixel is missing");
...
void writeHeaderAndDirectory(
    ...
    if (fseeko(output, 0, SEEK_SET) != 0 || !fwriteAll(output, bytes.data(), bytes.size())) {
        throw std::runtime_error("Unable to publish WebP index directory");
    }
    if (fflush(output) != 0 || fsync(fileno(output)) != 0) {
        throw std::runtime_error("Unable to sync WebP index");
    }
}
...
MappedFile mapFile(const std::string& path, bool writable, uint64_t requestedBytes = 0) {
    MappedFile mapped;
    const int flags = writable ? (O_RDWR | O_CREAT | O_TRUNC | O_CLOEXEC) : (O_RDONLY | O_CLOEXEC);
    mapped.fd = ::open(path.c_str(), flags, 0600);
    if (mapped.fd < 0) throw std::runtime_error("Unable to open a WebP build file");
    uint64_t bytes = requestedBytes;
    if (writable) {
        if (bytes == 0 || bytes > static_cast<uint64_t>(std::numeric_limits<off_t>::max()) ||
            ftruncate(mapped.fd, static_cast<off_t>(bytes)) != 0) {
            throw std::runtime_error("Unable to allocate temporary WebP raster storage");
        }
    } else {
        struct stat fileStat{};
        if (fstat(mapped.fd, &fileStat) != 0 || fileStat.st_size <= 0) {
            throw std::runtime_error("Unable to inspect the WebP source");
        }
        bytes = static_cast<uint64_t>(fileStat.st_size);
    }
    if (bytes > std::numeric_limits<size_t>::max()) {
        throw std::runtime_error("WebP build file is too large for this ABI");
    }
    mapped.size = static_cast<size_t>(bytes);
    const int protection = writable ? (PROT_READ | PROT_WRITE) : PROT_READ;
    void* address = mmap(nullptr, mapped.size, protection, writable ? MAP_SHARED : MAP_PRIVATE, mapped.fd, 0);
    if (address == MAP_FAILED) throw std::runtime_error("Unable to map a WebP build file");
    mapped.data = static_cast<uint8_t*>(address);
    return mapped;
}

bool fourCcEquals(const uint8_t* bytes, const char* value) {
    return memcmp(bytes, value, 4) == 0;
}

bool containsAnimation(const uint8_t* data, size_t size) {
    if (size < 12 || !fourCcEquals(data, "RIFF") || !fourCcEquals(data + 8, "WEBP")) return false;
    size_t offset = 12;
    while (offset <= size - 8) {
        const uint32_t chunkBytes = getU32(data, offset + 4);
        if (fourCcEquals(data + offset, "ANIM") || fourCcEquals(data + offset, "ANMF")) return true;
        const uint64_t next = static_cast<uint64_t>(offset) + 8ULL + chunkBytes + (chunkBytes & 1U);
...
std::pair<uint32_t, uint32_t> probeWebp(const uint8_t* data, size_t size) {
    if (containsAnimation(data, size)) {
        throw std::runtime_error("Animated WebP requires an animation-aware index");
    }
    int width = 0;
    int height = 0;
    if (!webpApi().getInfo(data, size, &width, &height) || width <= 0 || height <= 0) {
        throw std::runtime_error("Unable to parse the WebP source");
    }
...
void decodeBaseLevel(
    ...
    MappedFile source = mapFile(sourcePath, false);
    const auto dimensions = probeWebp(source.data, source.size);
    if (dimensions.first != level.width || dimensions.second != level.height) {
        throw std::runtime_error("WebP dimensions changed during index creation");
    }
    const uint64_t rowBytes64 = static_cast<uint64_t>(level.width) * 4;
    uint64_t rawBytes = 0;
    if (rowBytes64 > static_cast<uint64_t>(std::numeric_limits<int>::max()) ||
        mulOverflow(rowBytes64, level.height, &rawBytes)) {
        throw std::runtime_error("WebP raster dimensions overflow this ABI");
    }
    MappedFile raw = mapFile(rawTemporaryPath, true, rawBytes);
    if (webpApi().decodeRgbaInto(
            source.data,
            source.size,
            raw.data,
            raw.size,
            static_cast<int>(rowBytes64)
        ) == nullptr) {
        throw std::runtime_error("libwebp could not decode the source into the temporary raster");
    }
    ...
    for (uint32_t bandTop = 0; bandTop < level.height; bandTop += kTileSize) {
        ...
        uint8_t* band = raw.data + static_cast<size_t>(bandTop) * static_cast<size_t>(rowBytes64);
        for (uint32_t row = 0; row < bandRows; ++row) {
            premultiplyRow(band + static_cast<size_t>(row) * static_cast<size_t>(rowBytes64), level.width);
        }
        writeBand(output, level, entries, band, bandTop, bandRows);
    }
}
...
std::array<jint, 4> buildIndex(
    ...
    MappedFile probe = mapFile(sourcePath, false);
    const auto dimensions = probeWebp(probe.data, probe.size);
    std::vector<Level> levels = makeLevels(dimensions.first, dimensions.second);
    ...
    if (!output) throw std::runtime_error("Unable to create the WebP index");
    if (fseeko(output.get(), static_cast<off_t>(header.payloadOffset), SEEK_SET) != 0) {
        throw std::runtime_error("Unable to reserve the WebP index directory");
    }

    const std::string rowTemporaryPath = destinationPath + ".rgba";
    try {
        decodeBaseLevel(
            sourcePath,
            rowTemporaryPath,
        ...
        if (end < 0) throw std::runtime_error("Unable to determine WebP index size");
...
Java_io_github_indexedwebp_IndexedWebpNative_buildIndex(
    JNIEnv* env,
    jobject,
...
Java_io_github_indexedwebp_IndexedWebpNative_validateIndex(
    JNIEnv* env,
    jobject,
...
Java_io_github_indexedwebp_IndexedWebpNative_open(
    JNIEnv* env,
    jobject,
    jstring indexPathValue,
    jlong sourceBytes,
...
extern "C" JNIEXPORT jboolean JNICALL
Java_io_github_indexedwebp_IndexedWebpNative_decode(
    JNIEnv* env,
    jobject,
    jlong handle,
    jint left,
        ...
        for (uint32_t y = 0; y < expectedHeight; ++y) {
            ...
            if (selected->sample == static_cast<uint32_t>(sampleSize)) {
                ...
                while (x < expectedWidth) {
                    ...
                    if (sourcePixel == nullptr) throw std::runtime_error("WebP index pixel is missing");
...
Java_io_github_indexedwebp_IndexedWebpNative_close(JNIEnv*, jobject, jlong handle) {

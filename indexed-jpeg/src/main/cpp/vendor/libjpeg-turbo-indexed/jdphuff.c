        ...
        huffman_index *index, int scan_no, int offset)
{
  ...
  if (scan_no >= index->scan_count) {
    int old_scan_count = index->scan_count;
    index->scan = realloc(index->scan,
                    (scan_no + 1) * sizeof(huffman_scan_header));
    memset(index->scan + old_scan_count, 0,
          (scan_no - old_scan_count + 1) * sizeof(huffman_scan_header));
  ...
  free(index->scan[scan_no].offset);
  index->scan[scan_no].offset = (huffman_offset_data**)calloc(
